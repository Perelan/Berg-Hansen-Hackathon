...
function add_car($data){
    ...
    $car = array(
      "PickUpDateTime" => $session['VehRentalCore']['PickUpDateTime'],
      "ReturnDateTime" => $session['VehRentalCore']['ReturnDateTime'],
      "Fare" => $session['VehVendorAvails']['VehVendorAvail'][0]['VehAvailCore']['VehicleCharges']['VehicleCharge']['TotalCharge']['Amount']
    );
...
function overprise($data, $limit){
  ...
  foreach ($data as $dest){
    if(($dest['Fare'] + $dest['Car']['Fare']) < $limit && $dest['Car']['Fare'] != null){
